...
import { Children, cloneElement, forwardRef, isValidElement, useCallback, useEffect, useRef, useState, version as reactVersion } from 'react';
...
const VariantManagement = /*#__PURE__*/forwardRef((props, ref) => {
  ...
  const handleSaveView = (e, selectedVariant) => {
    if (typeof onSaveAs === 'function') {
      ...
      onSaveAs(enrichEventWithDetails(e, {
        ...selectedVariant,
        nativeDetail: e.detail.originalEvent.detail
      }));
    }
  ...
  const handleSaveManageViews = (e, payload) => {
    ...
    setSafeChildren(prev => Children.toArray(prev.map(child => {
      ...
      let updatedProps = {};
      const currentVariant = popoverRef.current.querySelector(`ui5-li[data-children="${CSS.escape(castChild.props.children)}"]`);
