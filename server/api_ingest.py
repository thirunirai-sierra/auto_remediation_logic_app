# api_ingest.py - Ingestion router for main FastAPI server
import sys
...
# Ensure HANA table exists
if hana_client:
    # Ensure watermark table exists (run once)
    try:
...
def query_log_analytics_range(start: datetime, end: datetime) -> list:
    """Query Log Analytics for failed runs in a time range (max 7 days)."""
...
async def ingest_incremental():
    """Pull new failures from Log Analytics since last watermark."""
    if not hana_client:
...
async def backfill(days: int = Query(30, description="Number of days to backfill (max 30)")):
    """
    Full backfill of recent failures from Log Analytics.
    Uses optimized 7-day chunks.
    """
    if not hana_client:
        raise HTTPException(503, "HANA client not available")
    ...
    # Use the watermark checkpoint if available
    ...
    total_inserted = 0
    chunk_size_days = 7  # Log Analytics supports max 7 days per query
    current = start_date
